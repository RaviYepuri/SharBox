    ...
    public class FolderLogController : Controller
    {
        ...
        public ActionResult List(int? page, string search = "", string pagesize = "")
        {
            try
            {
                ...
                logs = activityLogs.Where(x => x.FolderId > 0).OrderByDescending
        ...
        public void GetAllActivityLogsAsync(ActivityLogModel model)
        {
            try
            {
                ...
                IPagedList<ActivityLog> logs = null;
                IEnumerable<ActivityLog> activityLogs = _xmlLogService.GetActivityLogs().Where(x=>x.FolderId > 0);
                ...
                switch (model.SortColumn)
                {
                    case "CreatedOn":
                        if (model.SortOrder.Equals("desc"))
                            ...
                            logs = activityLogs.Where(x => x.FolderId > 0).OrderBy
                                    (m => m.CreatedOn).ToPagedList(model.CurrentPage, model.RecordsPerPage);
                        break;
                    case "DataRoomName":
                        if (model.SortOrder.Equals("desc"))
