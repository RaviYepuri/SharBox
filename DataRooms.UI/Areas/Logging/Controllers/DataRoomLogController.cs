    ...
    public class DataRoomLogController : Controller
    {
        ...
        public ActionResult List(int? page, string search="",string pagesize = "")
        {
            try
            {
                ...
                logs = activityLogs.Where(x => x.DataRoomId > 0).OrderByDescending
        ...
        public void GetAllActivityLogsAsync(ActivityLogModel model)
        {
            try
            {
                ...
                logs = activityLogs.Where(x => x.DataRoomId > 0).OrderByDescending
                                    (m => m.Id).ToPagedList(model.CurrentPage, model.RecordsPerPage);
                ...
                //        else
                //            logs = activityLogs.Where(x => x.DataRoomId > 0).OrderBy
                //                    (m => m.CreatedOn).ToPagedList(model.CurrentPage, model.RecordsPerPage);
                //        break;
                //    case "DataRoomName":
                //        if (model.SortOrder.Equals("desc"))
                ...
                model.PagedActivityLogs = logs;//.ToPagedList(model.CurrentPage, model.RecordsPerPage); ;
            }
            catch (Exception ex)
            {
                throw ex;
